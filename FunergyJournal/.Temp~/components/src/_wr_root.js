


	import * as mod0 from "wrm\/comp\/ViewComponentService";
	wrm.defineModule("wrm\/comp\/ViewComponentService", mod0)

	import * as mod1 from "wrm\/comp\/MessageService";
	wrm.defineModule("wrm\/comp\/MessageService", mod1)

	import * as mod2 from "wrm\/comp\/ListService";
	wrm.defineModule("wrm\/comp\/ListService", mod2)

	import * as mod3 from "wrm\/comp\/DetailsService";
	wrm.defineModule("wrm\/comp\/DetailsService", mod3)

	import * as mod4 from "wrm\/comp\/SelectorService";
	wrm.defineModule("wrm\/comp\/SelectorService", mod4)

	import * as mod5 from "wrm\/comp\/SwitchService";
	wrm.defineModule("wrm\/comp\/SwitchService", mod5)

	import * as mod6 from "wrm\/comp\/CreateService";
	wrm.defineModule("wrm\/comp\/CreateService", mod6)

	import * as mod7 from "wrm\/comp\/DeleteService";
	wrm.defineModule("wrm\/comp\/DeleteService", mod7)

	import * as mod8 from "wrm\/comp\/UpdateService";
	wrm.defineModule("wrm\/comp\/UpdateService", mod8)

	import * as mod9 from "testLocaleComponent\/ChangeLocaleService";
	wrm.defineModule("testLocaleComponent\/ChangeLocaleService", mod9)

	import * as mod10 from "wrm\/comp\/BarcodeOperationService";
	wrm.defineModule("wrm\/comp\/BarcodeOperationService", mod10)

	import * as mod11 from "wrm\/comp\/RestfulRequestResponseService";
	wrm.defineModule("wrm\/comp\/RestfulRequestResponseService", mod11)

